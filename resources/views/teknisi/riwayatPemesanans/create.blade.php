...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.riwayatPemesanan.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("teknisi.riwayat-pemesanans.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('pemesanan') ? 'is-invalid' : '' }}" name="pemesanan_id" id="pemesanan_id" required>
                    @foreach($pemesanans as $id => $entry)
                        <option value="{{ $id }}" {{ old('pemesanan_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('teknisi') ? 'is-invalid' : '' }}" name="teknisi_id" id="teknisi_id">
                    @foreach($teknisis as $id => $entry)
                        <option value="{{ $id }}" {{ old('teknisi_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status" required>
                    ...
                    @foreach(App\Models\RiwayatPemesanan::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', 'Menunggu Teknisi') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
