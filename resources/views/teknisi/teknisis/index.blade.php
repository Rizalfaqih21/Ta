...
@can('teknisi_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('teknisi.teknisis.create') }}">
                ...
                <tbody>
                    @foreach($teknisis as $key => $teknisi)
                        <tr data-entry-id="{{ $teknisi->id }}">
                            ...
                            <td>
                                @can('teknisi_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('teknisi.teknisis.show', $teknisi->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('teknisi_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('teknisi.teknisis.edit', $teknisi->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('teknisi_delete')
                                    <form action="{{ route('teknisi.teknisis.destroy', $teknisi->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('teknisi_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('teknisi.teknisis.massDestroy') }}",
