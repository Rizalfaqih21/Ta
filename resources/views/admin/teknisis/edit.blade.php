            ...
            <form method="POST" action="{{ route('admin.teknisis.update', [$teknisi->id]) }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id"
                        ...
                        @foreach ($users as $id => $entry)
                            <option value="{{ $id }}"
                                {{ (old('user_id') ? old('user_id') : $teknisi->user->id ?? '') == $id ? 'selected' : '' }}>
                                {{ $entry }}</option>
                        @endforeach
                    </select>
                    @if ($errors->has('user'))
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('no') ? 'is-invalid' : '' }}" type="text" name="no"
                        id="no" value="{{ old('no', $teknisi->no) }}" required>
                    @if ($errors->has('no'))
                ...
                <div class="form-group">
                    ...
                    <img id="previewImage" class="mb-2" src="{{ $kampanye->getImage() }}" width="60%" alt="">
